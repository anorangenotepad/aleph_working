...
void module_init(void) {
  ...
  param_setup(eParam_adc0, PARAM_AMP_MAX );
  param_setup(eParam_adc1, 0x00000000 );
  param_setup(eParam_adc2, PARAM_AMP_MAX );
  param_setup(eParam_adc3, 0x00000000 );
...
void module_process_frame(void) { 
  ...
  */
  ...
  //3) ring_fader attempt
  //   fader == 0, only A
  //   fader == MAX, only B
  //   fader > 0 && fader < MAX, A * B

  //first, need to set up fader
  //and make it applicable to all amps

  adcVal[0] = sub_fr1x32( adcVal[0], faderValue );
  adcVal[1] = add_fr1x32( adcVal[1], faderValue );
  adcVal[2] = sub_fr1x32( adcVal[2], faderValue );
  adcVal[3] = add_fr1x32( adcVal[3], faderValue );

  //then, need to have a conditional to only ouput A or B at opposite ends
  //and multiply when in the range between greater than 0 and less than MAX

  if (faderValue == PARAM_CV_MAX)
  {
    inputB = mult_fr1x32x32( in[1], adcVal[1] );
    finalOut = add_fr1x32( finalOut, inputB);
  }

  else if ( faderValue > 0 && faderValue < PARAM_CV_MAX )
  {
    inputA = mult_fr1x32x32( in[0], adcVal[2] );
    inputB = mult_fr1x32x32( in[1], adcVal[3] );
