...
extern void fill_param_desc(ParamDesc* desc) {
  ...
  //for ring switch/toggle on/off
  //got it from run_write0 example in lines/params.c
  //can see all param types in aleph/common/param_common.h
  //(need to check other dsp for syntax though...)
  strcpy(desc[eParam_ringSwitch].label, "ringSwitch");
  desc[eParam_ringSwitch].type = eParamTypeBool;
  desc[eParam_ringSwitch].min = 0;
  desc[eParam_ringSwitch].max = 1;
  desc[eParam_ringSwitch].radix = 2;

  //for fader value
  //this one is just a recycled cv entry from
  //above
  strcpy(desc[eParam_faderValue].label, "faderValue");
  desc[eParam_faderValue].type = eParamTypeFix; 
  desc[eParam_faderValue].min = 0x00000000; 
  desc[eParam_faderValue].max = PARAM_CV_MAX ;
  desc[eParam_faderValue].radix = 16; // full-scale 16.16
