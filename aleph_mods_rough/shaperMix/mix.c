...
//MY SHAPER VARIABLE WITH SAME NAME AS PARAM
static fract32 wShapeAdd;
static fract32 wShapeSub;
...
void module_init(void) {
  ...
  param_setup(eParam_wShapeAdd, 0);
  param_setup(eParam_wShapeSub, 0);
...
void module_process_frame(void) { 
  ...
  //this isnt right, but it complies
  //if you can find a blackfin intrinsic that gets 
  //you close, you migth be able to get something together
  //this just creates the master volume control...
  //adcVal[0] = mult_fr1x32x32( &(adcSlew[0]), wShape);

  //this doesnt do what you want to do...	
  //if (adcVal[0] > PARAM_AMP_MAX/2)
  //{
    //adcVal[0] = PARAM_AMP_MAX/2;
  //}
  ...
  */
  ...
  // NO JUST SCALE THE FIRST INPUT AND ADD TO BUS (this works!)
  ...
  //this does not work.. really poppy and shaping starts in middle and goes away again...
  //outBus = sub_fr1x32( outBus, wShape*10);

  //this does work, but shaping does not start until around the top of the scale
  //(i.e. 32k or so...)
  //outBus = sub_fr1x32( outBus, wShape);

  // copy the bus to all the outputs
  // NO JUST COPY OUTPUT BUS TO FIRST OUTPUT (this works too!)
  
  //NOTE: when you add, it cuts off the top of the waveform
  //      when you subtract, it cuts off the bottom of the waveform
  //      you want to do both!
  //      BUT! if you use the same control to do both, they will cancel each other out

  outBus = add_fr1x32( outBus, add_fr1x32(in[0], wShapeAdd) );
  outBus = add_fr1x32( outBus, sub_fr1x32(in[0], wShapeSub) );
  

  // current setting will 'shape' (cut off) top
  // change add to sub below to 'shape' bottom of waveform  
  //out[0] = add_fr1x32(in[0], wShape);
...
//=====================================================
//here is where the shaper functions (adapted from waves/param_set.c) go
//DONT NEED RIGHT NOW!
