...
void module_init(void) {
  ...
  //note: used to be PARAM_AMP_0 in original 
  //i changed to PARAM_AMP_MAX
  // also had to add PARAM_AMP_MAX to params.h and define (line 18)
  // (it also needed to go in params.c)
  param_setup( eParam_noteHz, 220 << 16 );
  param_setup( eParam_noteTune, FIX16_ONE );
  //it seems like PARAM_AMP_MAX >> 2 will set level to about half of max
  param_setup( eParam_noteLevel, PARAM_AMP_MAX >> 2);
...
void module_process_frame(void) {
  ...
  //see aleph/dsp/osc_polyblep.h for available options (square, saw, sine, triangle) and function definitions
  //you could change this to either square_polyblep, or saw_polyblep (because the functions are very similar i.e. both take two arguments)
  //there are also sine and triangle, but they appear to only take one argument, so would be necessary to re-write below code to account for that...
  fract32 sawNote = shl_fr1x32(square_polyblep(phasor_next(&noteOsc), noteOsc.freq),
			       15);
  fract32 myEnv = env_adsr_next(&noteEnv);
  fract32 filterEnvVal = env_adsr_next(&filterEnv);
  
  //i changed this because there was no output for me with myEnv included...
