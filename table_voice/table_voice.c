...
//table (rawsc/osc.c)
const fract32 wave_table[1024] = {
#include "wavetab_inc.c"
};
...
//sine
fix16 vHz[NUM_OSCS];
fix16 vTune[NUM_OSCS];
static filter_1p_lo vLevSlew[NUM_OSCS];
static fract32 vLevel[NUM_OSCS];
//phasor sinOsc;
...
void module_init(void) {
	...
	//Individual voices
	//sine
	//ref --> param_setup( eParam_sinHz0, 220 << 16 );
	/* below is from osc.c
     likely there was no eParamTypeNote, so
     it was an eParamTypeFix
     i like note better though, so 
     changed it.
     keeping code just in case...
  */
  //param_setup( eParam_sinHz0, FIX16_ONE );
  param_setup( eParam_vHz0, 220 << 16 );
  param_setup( eParam_vTune0, FIX16_ONE );

	//leave below params alone for now (should work)
	filter_1p_lo_init( &(vLevSlew[0]), 0 );
	param_setup( eParam_vLevSlew0, PARAM_SLEW_DEFAULT );
	param_setup( eParam_vLevel0, PARAM_AMP_MAX >> 2 );
	//phasor_init( &sinOsc );
...
void wave_voice_process(u16 idx) {
  ...
  u32 idxA;
	...
	waveA = shr_fr1x32(wave_table[idxA], 1);
	waveB = shr_fr1x32(wave_table[idxB], 1);

	// lookup, scale, and attenuate
  // just put attenuate control (sinLevel[0]) in here too
  // could move to out, but either way will work
  // ...maybe easier to follow code if up here?
  val = mult_fr1x32x32( vLevel[idx], add_fr1x32(mult_fr1x32x32(waveA, mulA),	mult_fr1x32x32(waveB, mulB)));

	// shift down to 24b
  // NO! DO NOT DO THIS! IT WILL MAKE VOLUME SUPER QUIET!!!!
	//val >>= 8;
	...
	out[0] = val; //ref --> mult_fr1x32x32(val, sinLevel[0]);
	out[1] = val;
	out[2] = val;
	out[3] = val;
...
void module_process_frame(void) {
	...
	vLevel[0] = filter_1p_lo_next( &(vLevSlew[0]) );

	//for reference
	//triLevel = filter_1p_lo_next( &(triLevSlew) );

	wave_voice_process(0);
...
void module_set_param(u32 idx, ParamValue v) {
	...
	switch(idx) {
		...
		case eParam_cvSlew3 :
			...
			//sine params
		case eParam_vHz0 : // --> upper 15 bit coarse
			vHz[0] = v;
      osc_set_phi_upper(0, v);
			//1. doesnt work --> osc_set_phi_upper(&(sinHz[0]), v);
			//2. ref         --> sinOsc.freq = fix16_mul_fract(sinTune, sinHz);
			break;
		case eParam_vTune0 : // --> lower 16 bits fine
			vTune[0] = v;
      osc_set_phi_lower(0, v);
      //1. osc_set_phi_lower(&(sinTune[0]), v);
			//2. sinOsc.freq = fix16_mul_fract(sinTune, sinHz);
			break;
			//no need to change these two (sinLevel, sinLevSlew)  
		case eParam_vLevel0 :
			filter_1p_lo_in( &(vLevSlew[0]), v);
			break;
		case eParam_vLevSlew0 :
			filter_1p_lo_set_slew( &(vLevSlew[0]), v);
			break;
