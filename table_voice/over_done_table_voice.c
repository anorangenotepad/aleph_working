...
fix16 sinHz[1];
fix16 sinTune[1];
static filter_1p_lo sinLevSlew[1];
static fract32 sinLevel[1];
//phasor sinOsc;
...
//osc.c function defs... probably overkill, but just in case...
...
void module_init(void) {
  ...
  //from rawsc
  //really need it otherwise too quiet
  param_setup( eParam_amp0, 0x7fffffff);

  //param_setup( eParam_sinHz0, 220 << 16 );
	param_setup( eParam_sinHz0,  0x00962fc9);
	param_setup( eParam_sinTune0, 0x00000000 );
    ...
    //case eParam_amp0 :
      ...
      //osc_set_amp(0, v);
		case eParam_sinHz0 : // --> upper 15 bit coarse
			sinHz[0] = v;
      //osc_set_phi(0, v); //not work at all...
