...
void module_process_frame(void) {
  ...
  else 
  {
    ...
    /* NOTE! DONT DO THIS!
     * YOU WANT TO ADJUST CONTROL PARAMETERS
     * IN THE SWITCH CASE BELOW (FOR DSP, but for OUT too probably)
     * (This module works with the code as shown in switch
     *  case below)
     *
    if (cvSwitch == 1)
    {
      cvVal[0] = PARAM_CV_VAL_DEFAULT;
    }
    else if (cvSwitch == 0 )
    {
      cvVal[0] = 0x00000000;
...
void module_set_param(u32 idx, ParamValue v) {
  ...
  //this shows how you can sucesfully control CV (DAC)   
  case eParam_cvSwitch :
    
    if ( v == 0 )
    {
      filter_1p_lo_in(&(cvSlew[0]), 0);
    }
    else if (v == 1)
    {
      filter_1p_lo_in(&(cvSlew[0]), PARAM_CV_VAL_DEFAULT);
    }
